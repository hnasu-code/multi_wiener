...
E_step_M2 = function(theta.hat,data,gammayes){
  ...
  for (i in 1:n){
    ...
    check.Sigma[1:p,((i-1)*p+1):(i*p)] = solve(Sigma_b.hat.inv+
                                                 tD.hat.inv[1:p,((i-1)*p+1):(i*p)])
  }
  
  # calculate check.mu
  check.mu = matrix(rep(0,n*p),ncol=n)
  for (i in 1:n){
    check.mu[1:p,i] = check.Sigma[1:p,((i-1)*p+1):(i*p)]%*%(Sigma_b.hat.inv%*%mu_b.hat+
                                                              D.hat.inv%*%dy_total[1:p,i])
  }
  
  # calculate Q function
  Q = 0
  for (i in 1:n){
    part2 = p*log(2*pi) + log(det(Sigma_b.hat)) + matrix.trace(Sigma_b.hat.inv%*%check.Sigma[1:p,((i-1)*p+1):(i*p)])+
      as.numeric(t(check.mu[1:p,i]-mu_b.hat)%*%Sigma_b.hat.inv%*%(check.mu[1:p,i]-mu_b.hat))
    Q = Q + part2
    for (j in 1:p){
      for (k in 1:m){
...
M_step_M2 = function(E.result,data,gammayes){
  ...
  for (i in 1:n){
    Sigma_b.sum = Sigma_b.sum + check.Sigma[1:p,((i-1)*p+1):(i*p)] + 
      (check.mu[1:p,i]-mu_b.hat)%*%t(check.mu[1:p,i]-mu_b.hat)
  }
  Sigma_b.hat = Sigma_b.sum/n  
  ...
  if (gammayes == 0){
    return(list(mu_b.hat=mu_b.hat,sigma_b.hat=sigma_b.hat,rho_b.hat=rho_b.hat,
                sigma_D.hat=sigma_D.hat,Sigma_b.hat=Sigma_b.hat))
  }else{
    ...
    return(list(mu_b.hat=mu_b.hat,sigma_b.hat=sigma_b.hat,rho_b.hat=rho_b.hat,
                sigma_D.hat=sigma_D.hat,gamma.hat=gamma.hat,Sigma_b.hat=Sigma_b.hat))
  }
}

## Define the implementation function-------------------------------------------
implementEM_M2 = function(theta.hat,data,gammayes) {
  if (gammayes == 0){
    theta.last = list(mu_b.hat = rep(0,p),sigma_b.hat=rep(0,p),
                      rho_b.hat=rep(0,p*(p-1)/2),
                      sigma_D.hat = rep(0,p),
  ...
  while(max(abs(unlist(theta.hat[-len]) - unlist(theta.last[-len]))) > 1e-5 &&  cur.iter <= max.iter) {
    theta.last = theta.hat
    E.result = E_step_M2(theta.last,data,gammayes)
    check.mu = E.result$check.mu
    check.Sigma = E.result$check.Sigma
    Q = E.result$Q
    theta.progress = rbind(theta.progress,as.data.frame(cbind(t(unlist(theta.last)),Q=Q)))
    
    theta.hat = M_step_M2(E.result,data,gammayes)
    progress(cur.iter,max.value = max.iter)
    cur.iter = cur.iter + 1
  }
  E.result = E_step_M2(theta.hat,data,gammayes)
  check.mu = E.result$check.mu
  check.Sigma = E.result$check.Sigma
  Q = E.result$Q
  theta.progress = rbind(theta.progress,as.data.frame(cbind(t(unlist(theta.hat)),Q=Q)))
  
  return(list(theta.progress=theta.progress,theta.hat=theta.hat,check.mu=check.mu,check.Sigma=check.Sigma,Q=Q))
}